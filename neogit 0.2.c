...
int run_config(int argc, char * argv[]){
    ...
    int t = argc - 2;
    if (strcmp(argv[t], "user.name") == 0){ halat = 0;
        strcpy(username, argv[t + 1]);
    }
    else if (strcmp ( argv[t], "user.email") == 0) {halat = 1;
        strcpy(email, argv[t + 1]);
    } else if ( strncmp ( argv[t], "alias", 5) == 0){ halat = 2;
        strcpy(argv[t], argv[t] + 6);
    }
    ...
    if (argc == 5 && strcmp(argv[2], "-global") == 0){
        ...
        while ( fgets(line, sizeof(line), ptr) != NULL){
            ...
            else { add_to_file(argv[t], ".neogit\alias") ; add_to_file(argv[t + 1], ".neogit\alias") ; }
            chdir(cwd);
        } fclose(ptr);
        ...
        find_name(".neogit");
        if (halat != 2 ) create_configs(username, email, halat);
        else {printf("!");  add_to_file(argv[t], ".neogit\alias") ; add_to_file(argv[t + 1], ".neogit\alias") ;}
        chdir(cwd);
    }  else {
