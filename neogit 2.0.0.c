...
int brace( char * add , int flag ){
    FILE * ptr = fopen ( add , "r");
    int min_p = 0, min_c = 0, min_b = 0, cur_p = 0 , cur_c = 0 , cur_b = 0; 
    char entry;
    while (( entry = fgetc ( ptr ) ) != EOF){
        if ( entry == '(') cur_p ++; if ( entry == ')') cur_p --;
        if ( entry == '[') cur_b ++; if ( entry == ']') cur_b --;
        if ( entry == '{') cur_c ++; if ( entry == '}') cur_c --;
        if ( min_p > cur_p) min_p = cur_p ; if ( min_b > cur_b) min_b = cur_b ; if ( min_c > cur_c) min_c = cur_c;
    } fclose ( ptr );
    if ( flag == 2 ){
        ptr = fopen ( add , "r");
        char out[100000] = "";
        for ( int i = 0 ; i < -min_p ; i++) { strcat ( out , "(") ; }
        for ( int i = 0 ; i < -min_b ; i++) { strcat ( out , "[") ; }
        for ( int i = 0 ; i < -min_c ; i++) { strcat ( out , "{") ; }
        char command[10000];int i = 0 ;
        while (( entry = fgetc ( ptr ) ) != EOF){
            command[i] = entry;
            i++;
        } command [i] = '\0';
        strcat ( out , command );
        for ( int i = 0 ; i < cur_p ; i++) { strcat ( out , ")") ; }
        for ( int i = 0 ; i < cur_b ; i++) { strcat ( out , "]") ; }
        for ( int i = 0 ; i < cur_c ; i++) { strcat ( out , "}") ; }
        fclose ( ptr) ; fopen ( add , "w") ; fprintf ( ptr, "%s" , out ); fclose ( ptr );
    }
    if ( min_b < 0 || min_c < 0 || min_p < 0 || cur_b != 0 || cur_c != 0 || cur_p !=0 ) return 1;
    return 0;
}
...
int word_in_file ( char * add , char * word , int flag) {
    int f = 0;
    for ( int k = 0 ; k < number_of_lines ( add ) ; k++){
        char line[1000]; out_line ( k , add , line );
        if ( strstr ( line , word) != NULL ){
            ...
            if ( flag == 2) {
                ...
                int i = 0 ; 
                char tmp [1000]; strcpy ( tmp , line);
                char * a =  ( char *) malloc ( sizeof ( char) * 1000);
                while ( strstr ( line , word) != NULL){
                    a = strstr ( line , word);
                    *(table + i) = a - line + i * strlen ( word );
                    int n = table[i] + strlen (word);
                    strcpy ( line , line + n - 1);
                    i++;
                }
                int j = 0;
                for ( int i = 0 ; i < strlen ( tmp ) ; i++){
                    if ( table [j] != i ){
                        ...
                        change_color ( 'b' ); printf("%s", word); reset_color(); i = i + strlen(word);
                        j = j + 1;
                    }
                } printf("\n");
            } if ( flag == 0 ) return 1 ;
...
int checker (char * hook_id , char * path , int flag ){
    ...
    else if ( strcmp ( hook_id , "balance-braces") == 0 && ( strcmp( format , "c")==0 || strcmp( format , "cpp")==0 || strcmp( format , "txt")==0) ){
        if ( brace( path , flag )){
            if ( flag ){
                printf ( "%s--------------------------------------------------------", hook_id); change_color('r'); printf ("FAILED\n"); return 0;
            }
...
int run_init(int argc, char * const argv[]) { //////////// hooks write
    char cwd[1024];
    ...
    if (!exists) {
        ...
        fprintf (file , "%s" , "file-size-check");fprintf (file , "%s" , "static-error-check");fprintf (file , "%s" , "format-check");
        fprintf (file , "%s" , "todo-check");fprintf (file , "%s" , "character-limit");fprintf (file , "%s" , "eof-blank-space");
        fprintf (file , "%s" , "balance-braces");
        fclose(file);
        file = fopen ( ".neogit/short-cuts", "w");
        fclose ( file );
...
int last_tree(const char *dirPath,  char * address, int n , char add2) { 
    DIR *dir; 
    struct dirent *entry; 
    if (!(dir = opendir(dirPath))) { 
        perror("opendir"); 
    ...
    while ((entry = readdir(dir)) != NULL) { 
        char path[1024]; 
        snprintf(path, sizeof(path), "%s\\%s", dirPath, entry->d_name); 
        if (entry->d_type == DT_DIR && n) { 
            if (strcmp(entry->d_name, ".") != 0 && strcmp(entry->d_name, "..") != 0 && strcmp ( entry->d_name, ".neogit") != 0) { 
                int x = search_for_line (path , add2);
                if ( x == -1 ){ printf ("%s : unstaged\n", path);}
                else { printf ("%s : staged\n", path);}
                last_tree(path, address,n - 1, add2); 
            } 
        } else {
            int x = search_for_line(path , address);
            if ( x == -1 ){ printf ("%s : unstaged\n", path);}
            else { printf ("%s : staged\n", path);}
            }
        } 
...
int run_add(int argc, char *const argv[]) { 
    ...
    if (argc < 3) { 
        ...
        return 1; 
    } else if ( argc == 3 && strcmp ( argv[2] , "-redo") == 0){
        char cwd[1000]; getcwd(cwd , sizeof ( cwd ));
        DIR * dir =  opendir ( ".neogit\\stage_area");
        struct dirent * entry ;
        while (( entry = readdir ( dir )) != NULL){
            if ( strcmp ( entry->d_name , ".") && strcmp ( entry->d_name , "..")){
                char add[1000] = "C:";
                char tmp_name[1000]; strcpy ( tmp_name , entry->d_name); change_word ( tmp_name, '^' ,'\\');
                strcat ( add , tmp_name );
                char command[1000];
                snprintf ( command , sizeof ( command ), "xcopy /Y .neogit\\stage_area\\%c%s%c %s", 34, entry->d_name, 34 , add );
                system ( command );
            }
        }
        char line [1000];
        char add[1000];
        for ( int i = 0 ; i < number_of_lines ( ".neogit/deleting") ; i++){
            ...
            if ( strcmp ( line , "*") ){
                ...
                *tmp = '\0';
                chdir (line);
                if ( is_file (add) == 1){ remove ( tmp  + 1 );}
                find_name ( ".neogit" );
            }
        } chdir ( cwd );
    }
    else if ( (argc == 3 ||  argc == 4 )  && strcmp ( argv[2] , "-n") == 0){
        char cwd[1000]; getcwd ( cwd ,sizeof ( cwd ));
        int n = 0 ; 
        if ( argc == 4 ) {
            n = number_out ( argv[3] );
        }
        last_tree (cwd, ".neogit/staging", n, ".neogit/tracks");
        return 1;
...
int run_commit_set(int argc, char * const argv[]){
    char cwd[1000]; getcwd ( cwd ,sizeof ( cwd ));
    if ( argc == 6 && strcmp (argv[2] , "-m") == 0 && strcmp ( argv[4], "-s") == 0 ){ printf ( "shortcut added" );
        find_name ( ".neogit" );
        add_to_file( argv[5], ".neogit/short-cuts");
        add_to_file ( argv[3], ".neogit/short-cuts");
    } else {
        printf ( "Invalid command" ); return 1;
    } chdir ( cwd );
}

int run_commit_reset ( int argc , char * const argv[]){
    char cwd[1000]; getcwd ( cwd ,sizeof ( cwd ));
    if ( argc == 6 && strcmp ( argv[2], "-m") == 0 && strcmp ( argv[4], "-s") == 0){
        find_name ( ".neogit" );
        int x = search_for_line ( argv[5], ".neogit/short-cuts");
        if ( x%2){
            printf ( "shortcut doesn't exist !");
        } else { printf ("shortcut changed");
            copy_file ( x + 1 , ".neogit/short-cuts", ".neogit/short-cusafdc", argv[3]);
        } chdir ( cwd );
    } else {
        printf ( "Invalid command" ); return 1;
    }
...
int run_commit_remove ( int argc , char * const argv[]){
    ...
    if ( argc == 4 && strcmp ( argv[2], "-s") == 0){
        find_name ( ".neogit" );
        int x = search_for_line ( argv [3],".neogit/short-cuts");
        if ( x%2) {
            printf ( "shortcut doesn't exist !");
        } else { printf ( "shortcut deleted" );
            delete_line ( ".neogit/short-cuts", x + 1);
            delete_line ( ".neogit/short-cuts", x + 1);
        } chdir ( cwd );
    } else {
        printf ( "Invalid command" ); return 1;
...
int run_commit(int argc, char * const argv[]) {
    ...
    if ( argc == 4 && strcmp ( argv[2], "-s") == 0){
        int x = search_for_line (argv[3] , ".neogit/short-cuts");
        if ( x%2 == 0){ char line[1000]; out_line ( x + 1 , ".neogit/short-cuts", line);
            char command[1000] = "neogit commit -m "; strcat ( command ,line );
            system ( command );
            return 0 ;
        } else {
            printf ("shortcut doesn't exist !"); return 1;
        }
    }
    if ( argc == 2){
...
int space_breaker( char * word ){ //////////////  bug 
...
int check_equalize (char * add1 , char * add2 , int start1, int start2 , int finish1 , int finish2 ){
    int cur1 = start1, cur2 = start2;

    char * name1 = strrchr ( add1 , '\\' ); 
    if ( name1 == NULL ) {name1 = add1;}
    else { name1 ++ ;}
    name1 = strrchr ( name1 , '^' ); 
    if ( name1 != NULL ) name1 ++ ;
    char * name2 = strrchr ( add2 , '\\' ); 
    if ( name2 == NULL ) {name2 = add2;}
    else { name2 ++ ;}
    name2 = strrchr ( name2 , '^' ); 
    if ( name2 != NULL ) name2 ++ ;
    int flag = 0;

    while ( (cur1 < finish1 ) && (cur2 < finish2 ) ){
        char line1[1000]; char line2[1000];
        char tmp1 [1000]; char tmp2[1000];
        for ( int i = cur1 ; i < finish1 ; i++ ){
            out_line ( i , add1 , line1 );
            strcpy ( tmp1 , line1 );
            cur1 = i ; 
            if ( space_breaker(line1) ) {
                break ;
            }
        } 
        for ( int i = cur2 ; i < finish2 ; i++){
            out_line ( i , add2 , line2 );
            strcpy ( tmp2 , line2 );
            cur2 = i ; 
            if ( space_breaker(line2) ) {
                break ;
            }
        }
        out_line ( cur1 , add1, line1 );
        out_line ( cur2 , add2, line2 );
...
int run_diff ( int argc , char * const argv[]){                        /////////////////////////// not compelet ////////////////
    ...
    if ( argc == 5 && strcmp ( argv[2] , "-c" ) == 0 ){
        find_name ( ".neogit" );
        char add1 [1000] = ".neogit\\commits\\"; strcat ( add1 , argv[3] );
        char add2 [1000] = ".neogit\\commits\\"; strcat ( add2 , argv[4] );

        FILE * ptr = fopen ( ".neogit/tmp1" , "w"); fclose ( ptr );
        ptr = fopen ( ".neogit/tmp2" , "w"); fclose ( ptr );

        tree(add1 , ".neogit/tmp1");
        tree(add2 , ".neogit/tmp2");

        char line[1000];
        char neo[1000]; getcwd ( neo , sizeof ( neo ) );



        for ( int i = 0 ; i < number_of_lines ( ".neogit/tmp1") ; i++){
            out_line ( i , ".neogit/tmp1" , line );
            strcpy ( line , line + strlen (neo) + strlen ( argv[3] ) + 16);
            copy_file ( i , ".neogit/tmp1", ".neogit/sfdpqwetptew83bycbgr" , line );

        }

        for ( int i = 0 ; i < number_of_lines ( ".neogit/tmp2") ; i++){
            out_line ( i , ".neogit/tmp2" , line );
            strcpy ( line , line + strlen (neo) + strlen ( argv[4] ) + 16);
            copy_file ( i , ".neogit/tmp2", ".neogit/sfdpqwetptew83bycbgr" , line );
        }
        change_word ( neo , '\\' , '^');
        strcpy ( neo , neo + 2);
        for ( int i = 0 ; i < number_of_lines ( ".neogit/tmp1") ; i++){
            out_line ( i , ".neogit/tmp1" , line );
            int x = search_for_line ( line , ".neogit/tmp2");
            if ( x == -1 && strlen ( line ) > 1){
                change_word ( line , '^', '\\');
                printf ("the file %s doesn't exist in commit-id:%s\n" , line , argv[4] );
            } else if (strlen ( line ) > 1){
                char command [1000];
                snprintf ( command , sizeof ( command ), "neogit diff -f %c.neogit\\commits\\%s\\%s^%s%c %c.neogit\\commits\\%s\\%s^%s%c",34,
                argv[3],neo, line,34 ,34, argv[4],neo, line,34 );
                system ( command );
            }
        }
        ...
        for ( int i = 0 ; i < number_of_lines ( ".neogit/tmp2") ; i++){
            out_line ( i , ".neogit/tmp2" , line );
            int x = search_for_line ( line , ".neogit/tmp1");
            if ( x == -1 && strlen ( line ) > 1){
                change_word ( line , '^', '\\');
                printf ("the file %s doesn't exist in commit-id:%s\n" , line , argv[3] );
            } 
    ...
    if ( argc < 5 || strcmp ( argv[2] , "-f" ) || ( argc != 5 && argc != 9 )){
        ...
        finish1 = number_of_lines ( file1_add ) ; finish2 = number_of_lines ( file2_add ) ;
        start1 = 0 , start2 = 0 ;
        debug ( file1_add )
        
    } else if ( argc == 9 && strcmp (argv[5], "-line1") == 0 && strcmp ( argv[7], "-line2") == 0 ) {
        ...
        int start1 = number_out ( numbers[0] );
        int finish1 = number_out ( numbers[1] );
        ...
        int start2 = number_out ( numbers[2] );
        int finish2 = number_out ( numbers[3] );

    }
    int x = check_equalize (file1_add ,file2_add , start1 , start2 , finish1 , finish2 );
...
int run_grep ( int argc, char * const argv[] ){
    ...
    if ( argc >= 6 && strcmp ( argv[2] , "-f") == 0 && strcmp ( argv[4], "-p") == 0 ){
        int flag = 1;
        if (strcmp (argv [argc - 1] , "-n") == 0) flag = 2;
        if ( argc > 7 && strcmp ( argv[6], "-c") == 0 && strcmp ( argv[7] , "-n")){
            find_name(".neogit"); 
            chdir ( ".neogit\\commits");
            chdir ( argv[7] );
            char add[1000];
            if ( strncmp ( add , "C:\\" ,3) ) { strcpy ( add , cwd ) ; strcat ( add, "\\"); strcat ( add , argv[3]);}
            change_word ( add , '\\', '^'); strcpy ( add , add + 2 ); 
            word_in_file ( add, argv[5], flag); 
            chdir ( cwd );
        } else if ( argc == 6 || ( argc == 7 && strcmp ( argv[6], "-n") == 0)) word_in_file( argv[3] , argv[5], flag);
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(argv[1], "init") == 0) {
        ...
        return run_commit_reset ( argc , argv);
