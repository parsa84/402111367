...
void tree( char * word){
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (entry->d_type == DT_DIR){
            ...
            chdir(cwd);
            tree(word);
        } else if (strcmp(entry->d_name, word) == 0){
            printf("%s\n", cwd);
        }
    chdir(current);
    }
...
int can_add_stage(char * name, char * file_add){
    FILE * ptr = fopen(file_add, "r");
    char line[1000];
    int i = 2 ; 
    while ( fgets(line, sizeof(line), ptr) != NULL){ line[strlen(line) - 1] = '\0';
        if (strncmp( name , line, min (strlen(name), strlen( line )) ) == 0){
            if ( strlen( name) >= strlen ( line)) return  0 ; //name pesar hast
            return i; // name pedar hast
        } i ++;
    }
    return 1;
}
...
int run_add(int argc, char *const argv[]) { 
    ...
    if (argc < 3) { 
        ...
        if ( is_file(cwd) != -1){
            ...
            find_name(".neogit");
            char path[1000];
            int lenth = strlen(getcwd(path, sizeof(path)));
            int i = can_add_stage(cwd, ".neogit/staging");
            if (i != 0){
                add_to_file(cwd, ".neogit/staging"); char path2[1000];
                strcpy( path, cwd); strcpy( path, path + lenth); strcpy ( path2, ".neogit\\stage_area"); strcat(path2, path);
                add_to_file(path2, ".neogit/relstaging");  
                char systmp[1000] = "xcopy "; 
                strcat(systmp, cwd); 
                printf("%s\n", systmp);
                strcat(systmp, " .neogit\\stage_area");
                if ( flag ) strcat(systmp, "/Y");
                else {strcat(systmp, "\\"); strcat(systmp, tmp + 1);
                    *tmp = '\0'; chdir(".neogit/stage_area");  mkdir(tmp + 1); find_name(".neogit");
                strcat(systmp, "/E /Y"); }
                printf("this is it: %s \n", systmp); 
                system(systmp);
            } if ( i > 1){ i = i - 2;
                ...
                perror("you have a file or directory already staged");
            }chdir(tmpcwd);
        } else {
            char string [100];
            strcpy( string , argv[2]); strcat ( string, " ");
            perror(string);
        }
    } 
    else { 
