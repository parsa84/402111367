    ...
    if (argc < 3) { 
        ...
        if ( is_file(cwd) != -1){
            ...
            while ( fgets( entry, sizeof( entry), ptr_rel) != NULL){ entry[strlen(entry) - 1] = '\0'; 
                snprintf(command, sizeof(command), "xcopy /Y %s .neogit\\stage_area", entry);
                printf("%s", command);
                system(command);
                ...
                for ( int i = 0 ; i < j ; i++){
                    copy_file( lines[i], ".neogit/staging", "sfd", "");
           ...
           /* tmp = strrchr(cwd, '\\'); 
            ...
            chdir(cwd); // mire be directory
            getcwd(cwd, sizeof(cwd)); 
            if (flag) { // cwd hala address dorost hast.
                *tmp = '\\'; 
            } 
            find_name(".neogit");
            char path[1000];
            int lenth = strlen(getcwd(path, sizeof(path)));
            char address [1000]; strcpy( address, cwd); */
